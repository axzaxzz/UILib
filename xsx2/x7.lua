...
function library:GetWorkspaceFolder()
    ...
    -- Try multiple methods to detect workspace
    if isfolder then
        -- Method 1: Check common executor folders
        local commonPaths = {
            "workspace",
            "scripts",
            "autoexec",
    ...
    -- Method 2: Try to detect from current script path
    if not workspaceFolder and getscriptclosure then
...
function library:ListThemeFiles()
    ...
    if listfiles then
        local workspace = library:GetWorkspaceFolder()
        local searchPath = workspace ~= "" and workspace or "."
        
        local success, files = pcall(function()
            if isfolder and isfolder(workspace) then
