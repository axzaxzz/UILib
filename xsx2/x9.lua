...
local function LoadImageFromURL(url, filename)
    filename = filename or "temp_image.png"
    
    -- Check if functions exist
    if not assetFunctions.writefile or not assetFunctions.HttpGet then
        return nil, "missing file functions"
    end
    
    -- Try to download and save image
    local success, result = pcall(function()
        -- Check if already cached
        if assetFunctions.isfile and assetFunctions.isfile(filename) then
            -- Use cached version
            if assetFunctions.getcustomasset then
                return assetFunctions.getcustomasset(filename)
            elseif assetFunctions.getsynasset then
                return assetFunctions.getsynasset(filename)
            end
        end
        ...
        -- Save to file
        assetFunctions.writefile(filename, imageData)
...
function library:Introduction()
    ...
    -- Image URL to load (replace with your desired image)
    ...
    local imageAsset, loadError = LoadImageFromURL(imageURL, "intro_image.png")
    local imageLoaded = false
    
    -- Try Drawing.new("Image") first if image was loaded successfully
    if imageAsset and drawingCapabilities.available and drawingCapabilities.Image then
    ...
    -- Try ImageLabel if Drawing.Image not available but asset loaded
    elseif imageAsset then
