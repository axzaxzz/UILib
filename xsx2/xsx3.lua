...
function library:CreateBackgroundOverlay()
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        for _, dotData in pairs(dots) do
            if dotData.obj and dotData.obj.Parent then
                -- Move down
                local currentY = dotData.obj.Position.Y.Scale
                local newY = currentY + dotData.speed
                
                if newY > 1 then
                    newY = -0.05
                    dotData.obj.Position = UDim2.new(math.random(0, 100) / 100, 0, newY, 0)
                else
                    dotData.obj.Position = UDim2.new(dotData.obj.Position.X.Scale, 0, newY, 0)
                end
                
                -- Fade in/out
                local currentTrans = dotData.obj.BackgroundTransparency
                local newTrans = currentTrans + (dotData.fadeSpeed * dotData.fadeDir)
                ...
                dotData.obj.BackgroundTransparency = math.clamp(newTrans, 0.3, 0.9)
            end
        end
    end)
end
